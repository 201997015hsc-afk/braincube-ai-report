...
CHART_COLORS = [BRAIN_CUBE_ORANGE, '#1F77B4', '#2CA02C', '#D62728', '#9467BD', '#8C564B', '#E377C2', '#7F7F7F', '#BCBD22', '#17BECF']
...
<style>
    ...
    .stMarkdown h1 {{ background: linear-gradient(135deg, {BRAIN_CUBE_ORANGE} 0%, #FFB75E 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 900; letter-spacing: -1px; }}
    ...
    [data-testid="metric-container"] {{ background-color: #FFFFFF; border: 1px solid #EBEBEB; padding: 15px 10px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.04); border-left: 6px solid {BRAIN_CUBE_ORANGE}; transition: transform 0.2s ease-in-out; }}
    [data-testid="metric-container"]:hover {{ transform: translateY(-3px); box-shadow: 0 6px 15px rgba(247, 147, 29, 0.15); }}
    .stMetric [data-testid="stMetricLabel"] {{ color: #777777; font-weight: 600; font-size: 1.0rem; }}
    .stMetric [data-testid="stMetricValue"] {{ color: #333333 !important; font-weight: 800 !important; font-size: clamp(1.1rem, 1.5vw, 1.7rem) !important; white-space: nowrap !important; word-break: keep-all !important; letter-spacing: -0.5px; }}
    .stButton button {{ background: linear-gradient(135deg, {BRAIN_CUBE_ORANGE} 0%, #E07D10 100%); color: white; border: none; border-radius: 8px; padding: 12px 24px; font-weight: 700; box-shadow: 0 4px 6px rgba(247, 147, 29, 0.3); transition: all 0.3s; }}
...
try:
    if os.path.exists("benchmark.csv"):
        ...
        for col in ['발송건', '클릭수', '광고비', 'DB']:
            if col in benchmark_df.columns:
                benchmark_df[col] = pd.to_numeric(benchmark_df[col].astype(str).str.replace(',', '').str.replace('-', ''), errors='coerce').fillna(0)
        if '분야' in benchmark_df.columns:
...
if excel_file is not None:
    ...
    with st.expander("⚙️ 데이터 항목 매칭 설정 (클릭하여 수정)"):
        ...
        col_cost = c4.selectbox("💰 광고비(소진액)", cols, index=get_idx(['cost_raw', '광고비', '소진액', '사용금액', '비용', 'cost', '금액']))
    ...
    try:
        ...
        if len(selected_date_range) == 2:
            ...
            # ==========================================
            # 🚨 [완벽 해결] 타이틀 겹침 방지 + 투명 파이차트를 이용한 완벽한 반응형 호버 툴팁!
            # ==========================================
            if target_budget > 0 or (has_db and target_cpa > 0):
                ...
                if target_budget > 0:
                    ...
                    fig_budget.add_trace(go.Indicator(
                        ...
                        value=total_spent, 
                        number={'valueformat': ',.0f'}, # 'k' 대신 콤마(,) 표시
                        delta={'reference': target_budget, 'position': "top", 'valueformat': ',.0f'},
                        domain={'x': [0, 1], 'y': [0, 0.75]}, # 💡 차트를 아래로 75%만 써서 위쪽 여백 확보 (겹침 방지)
                        title={'text': "총 예산 소진 현황 (원)<br><span style='font-size:12px;color:gray'>마우스를 올려보세요</span>", 'font': {'size': 16}},
                    ...
                    # 반응형 호버 툴팁을 위한 투명 도넛 차트 덮어쓰기 (레이아웃 안 깨짐)
                    fig_budget.add_trace(go.Pie(
                        values=[1], textinfo='none', hoverinfo='text',
                        hovertext=f"💰 현재 소진액: {total_spent:,.0f} 원<br>🎯 목표 예산: {target_budget:,.0f} 원<br>📊 소진율: {(total_spent/target_budget)*100 if target_budget>0 else 0:.1f}%",
                        hole=0.5, marker=dict(colors=['rgba(0,0,0,0)']), # 완전 투명
                ...
                if has_db and target_cpa > 0:
                    ...
                    fig_cpa.add_trace(go.Indicator(
                        ...
                        domain={'x': [0, 1], 'y': [0, 0.75]}, # 💡 겹침 완벽 방지
                    ...
                    g_col2.plotly_chart(fig_cpa, use_container_width=True)
            
            m_col1, m_col2, m_col3, m_col4 = st.columns(4)
            m_col1.metric("총 소진액", f"{total_spent:,.0f} 원")
            ...
            with tab1:
                ...
                with pie_col1:
                    if col_cost in ranking_df.columns:
                        fig_pie_cost = px.pie(ranking_df, values=col_cost, names=group_target, hole=0.4, title=f"💰 {group_target}별 소진액 비중", color_discrete_sequence=CHART_COLORS)
                        fig_pie_cost.update_traces(textposition='inside', textinfo='percent+label'); st.plotly_chart(fig_pie_cost, use_container_width=True); st.session_state.saved_charts['소진액 비중 (도넛)'] = fig_pie_cost
                with pie_col2:
            ...
            with tab4:
                ...
                if not heatmap_data.empty:
                    ...
                    else: heat_trend, heat_label, color_scale, text_format = col_cost, "소진액 (원)", "Oranges", '.0f'
            ...
            else: trend_col, y_label = col_cost, "소진액 (원)"
            ...
            display_metric_name = main_metric if main_metric in ['CPA', 'CTR', 'CPC'] else "소진액"
            ...
            if len(top_data) > 0:
                ...
                for i, (index, row) in enumerate(top_data.iterrows()):
                    ...
                    with cols[i]:
                        ...
                        st.write(f"- 소진액: {row[col_cost]:,.0f}원")
